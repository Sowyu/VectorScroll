...
const W = 1440, H = 900, FPS = 60, SECONDS = 12;
const DEAD = 10, GAIN = 0.11, MAX = 26;
const here = path.dirname(fileURLToPath(import.meta.url));
const frames = path.join(here, 'frames');

// Small deterministic hand tremor so the pointer never travels a perfect curve.
let seed = 11;
...
function pose(ms) {
  if (ms < 900) return { x: lerp(1180, 790, ease(seg(ms, 0, 900))), y: lerp(720, 445, ease(seg(ms, 0, 900))), down: false };
  if (ms < 1500) return { x: lerp(790, O.x, ease(seg(ms, 900, 1400))), y: lerp(445, O.y, ease(seg(ms, 900, 1400))), down: false };
  if (ms < 1800) return { ...O, down: true };                                                     // press, hesitate
  if (ms < 4800) return { x: O.x + 14 * seg(ms, 1800, 4800), y: O.y + 95 * ease(seg(ms, 1800, 3200)), down: true };
  if (ms < 6000) return { x: O.x + lerp(14, 9, seg(ms, 4800, 6000)), y: O.y + lerp(95, 40, ease(seg(ms, 4800, 6000))), down: true };
  if (ms < 9200) return { x: O.x + lerp(9, -6, seg(ms, 6000, 9200)), y: O.y + lerp(40, -150, ease(seg(ms, 6000, 7600))), down: true };
  if (ms < 10200) return { x: O.x + lerp(-6, 2, seg(ms, 9200, 10200)), y: O.y + lerp(-150, 3, ease(seg(ms, 9200, 10000))), down: ms < 10000 };
  return { x: lerp(O.x + 2, 1180, ease(seg(ms, 10400, 12000))), y: lerp(O.y + 3, 720, ease(seg(ms, 10400, 12000))), down: false };
}
...
for (let n = 0; n < total; n++) {
  const ms = n * 1000 / FPS;
  const p = pose(ms);
  const x = p.x + rnd() * 1.2, y = p.y + rnd() * 1.2;
...
run(['-i', path.join(frames, '0150.png'), '-q:v', '3', path.join(here, '..', 'poster.jpg')]);
rmSync(frames, { recursive: true, force: true });
console.log('done');
