...
extension VectorScrollApp {
    static func checkDelaySettings() -> Bool {
        ...
        assert(titles.contains("Scroll While Holding"))
        assert(titles.contains("Scroll Until Next Click"))
        assert(titles.contains("Hide Menu Bar Icon"))
        let appearance = subject.menu.items.first { $0.title == "Indicator Appearance" }!.submenu!
        assert(appearance.items.contains { $0.title == "Light Indicator" })
        assert(appearance.items.contains { $0.title == "Dark Indicator" })
        assert(appearance.items.contains { $0.title == "Indicator Size" })
        ...
        assert(subject.delayItem.title == "Hold Before Starting: 750 ms")
        ...
        assert(subject.holdToLockItem.title == "Scroll Until Next Click")
